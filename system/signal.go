...
const (
	...
	Maintenance
	// ShutdownSignalName defines 'SHUTDOWN'
	ShutdownSignalName = "SHUTDOWN"
	// ReloadSignalName defines 'RELOAD'
	ReloadSignalName = "RELOAD"
	// MaintenanceSignalName defines 'MAINTENANCE'
	MaintenanceSignalName = "MAINTENANCE"
)
...
func (s SignalType) String() string {
	...
	case Reload:
		return ReloadSignalName
	case Maintenance:
		return MaintenanceSignalName
	default:
	...
	for {
		...
		case sig := <-s.interrupt:
			...
			case isSignalAvailable(sig, s.shutdown):
				...
				err := operator.Shutdown(fmt.Errorf("service was terminated by system signal"))
				if err != nil {
					logger.Error(err)
				}
				s.quit <- struct{}{}
